<h1 align="center">⚙️🔥 Fernando | Fullmetal Developer 🔥⚙️</h1>

<p align="center">
  <img src="https://i.gifer.com/1AFF.gif" width="300px" />
</p>

<p align="center">
  "A troca equivalente não é apenas uma lei da alquimia, é uma lei da vida."
</p>

---

## 🧪 Sobre Mim

👋 Olá! Eu sou **Fernando**, um desenvolvedor apaixonado por tecnologia, programação e aprendizado contínuo.

Assim como na alquimia, acredito que:

> 📌 Para obter algo valioso, é preciso esforço, disciplina e estudo.

Meu objetivo é evoluir constantemente como programador e criar soluções inteligentes usando tecnologia.

---

## ⚙️ Tecnologias & Ferramentas

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,js,html,css,nodejs,linux,git,github,vscode" />
</p>

### 💻 Principais Skills

- 🐍 Python
- 🌐 JavaScript
- 🎨 HTML & CSS
- 🐧 Linux
- 🔐 Segurança / Hacking Ético
- 🤖 Automação (n8n, bots)
- 📊 Lógica de Programação

---

## 🔥 Filosofia: Alquimia + Programação

<p align="center">
  <img src="https://media.tenor.com/yr8Z0d3eCzYAAAAC/fullmetal-alchemist-edward.gif" width="350px"/>
</p>

Na alquimia:

> Matéria → Transmutação → Evolução

Na programação:

> Problema → Código → Solução

📈 Todo projeto é uma transmutação.

---

## 🏆 Estatísticas do GitHub

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=SEU_USUARIO&show_icons=true&theme=radical" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=SEU_USUARIO&theme=radical" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USUARIO&layout=compact&theme=radical" />
</p>

⚠️ Troque `SEU_USUARIO` pelo seu username do GitHub.

---

## 📂 Projetos em Destaque

### 🧠 Sistema de Automação
> Automação de processos usando Python + n8n

### 🔐 Estudos em Segurança
> Laboratórios de Pentest e redes

### 🤖 Bots Inteligentes
> WhatsApp, Telegram e APIs

📌 Veja todos nos meus repositórios!

---

## 🧬 Evolução Contínua

<p align="center">
  <img src="https://media.tenor.com/5j6H4U5B2UsAAAAC/fullmetal-alchemist-brotherhood.gif" width="350px"/>
</p>

✔️ Estudo diário  
✔️ Projetos reais  
✔️ Prática constante  
✔️ Mentalidade de crescimento  

---

## 🌍 Onde me Encontrar

<p align="center">
  <a href="https://github.com/SEU_USUARIO">
    <img src="https://img.shields.io/badge/GitHub-000?style=for-the-badge&logo=github" />
  </a>

  <a href="https://linkedin.com/in/SEU_LINKEDIN">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin" />
  </a>

  <a href="mailto:SEU_EMAIL@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail" />
  </a>
</p>

---

## 📜 Código do Alquimista

> ❝ Um programador que para de aprender, começa a falhar. ❞

> ❝ A verdadeira transmutação é transformar conhecimento em impacto. ❞

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=SEU_USUARIO&color=red&style=flat-square" />
</p>

<p align="center">
  ⚙️ Desenvolvido com disciplina, estudo e paixão por tecnologia.
</p>
